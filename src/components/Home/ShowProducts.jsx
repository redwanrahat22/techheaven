...
const ShowProducts = () => {
    ...
    return (
        <div className="">
           
            {
              ...
              <>
              <div className="text-center mt-12 font-menu">

              <div className="max-w-6xl mx-auto ">
      <div className="relative overflow-hidden rounded-lg shadow-lg">
        <div className="flex transition-transform duration-500" style={{ transform: `translateX(-${currentIndex * 100}%)` }}>
          {images.map((image, index) => (
            <img key={index} src={image} className="w-full object-cover" />
          ))}
        </div>
    ...
    <h1 className={`text-xl ${screenmode ? 'text-dmgreen': 'text-lmblue'} mt-20`}>Showcasing {brand.brand} Products</h1>
        

              </div>
              <div className="grid grid-cols-3 m-auto w-3/5 gap-6 mt-10 ">
              {
